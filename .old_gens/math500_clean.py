...
import subprocess
print("running on node:" + subprocess.run(["hostname"], capture_output=True, text=True).stdout.strip())
...
def question_prompt_template(q):
    return f'<|im_start|>system\nAnswer the question in short<|im_end|>\n<|im_start|>user\n {q}<|im_end|>\n<|im_start|>assistant\n <think>'
...
def create_math500_experiment() -> Experiment:
    ...
    model_config = ModelGenerationConfig(
        model_name="qwen-7B",
        model_path="/home/ADV_2526a/evyataroren/inter_2025/models/junk/facebook/MobileLLM-R1.5-140M",
        should_stop_fn=lambda tokens: False,
        get_injection_fn=lambda tokens: "",
...
def main():
    ...
    output_path = os.path.join(output_dir, f"math500_whole_clean_no_activations_{timestamp}.pkl")
