...
from pipeline.dataset_loaders import aggregated_dataset_loader, aggregate_shuffle_strategy, AI2ARCLoader
...
def question_prompt_template(q):
    return f"<|begin_of_sentence|><|User|> " \
...
def judge_prompt_fn(question, correct_answer, generated_text):
    return (
        ...
        f"You are a science teacher. Compare the model's answer with the correct answer.\n\n"
        f"Question: {question}\n\n"
...
def create_ai_arc_experiment() -> Experiment:
    """Create an experiment for AI2ARC dataset with attention capture."""
    ...
    experiment.datapoints = experiment.datapoints[task_id_str]
...
def main():
    print("=" * 80)
    print("Starting AI2ARC Experiment (With injection, Attention Capture)")
    print("=" * 80)
    
    # Create experiment
    print("\n1. Creating experiment configuration...")
    experiment = create_ai_arc_experiment()
    print(f"   Loaded {len(experiment.datapoints)} datapoints from AI2ARC")
    ...
    output_path = os.path.join(output_dir, f"ai_arc_with_injection_{timestamp}.pkl")
